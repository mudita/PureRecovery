    ...
    if ((1U == s_cdcVcom.attach) && (1U == s_cdcVcom.startTransactions))
    {
        ...
        if ((0 != s_recvSize) && (USB_CANCELLED_TRANSFER_LENGTH != s_recvSize))
        {
            ...
            if ((0U != s_recvSize) && (USB_CANCELLED_TRANSFER_LENGTH != s_recvSize))
            {
                ...
                s_recvSize = 0;
                if(s_currRecvBuf[0] == 'q'){
                    should_exit = true;
                    printf("Received quit request, exiting...");
                }else{
                    printf("rx: %s\n",s_currRecvBuf);
                }
            }
        ...
        if (0U != s_sendSize)
        {
            ...
            s_sendSize    = 0;
            const int len = snprintf(s_currSendBuf,sizeof s_currSendBuf,"\nping:%c",s_currRecvBuf[0]);
            error = USB_DeviceCdcAcmSend(s_cdcVcom.cdcAcmHandle, USB_CDC_VCOM_BULK_IN_ENDPOINT, s_currSendBuf, len);
